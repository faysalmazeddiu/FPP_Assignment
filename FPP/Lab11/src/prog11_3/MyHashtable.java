...
public class MyHashtable implements Iterable {
	...
	private void rehash() {
		...
		for (int index = 0; index < table.length; index++) {
			...
			Iterator iterateTable = table[index].iterator();
			while (iterateTable.hasNext()) {
				Entry entry = (Entry) iterateTable.next();
				int hashCodeNew = entry.key.hashCode();
				int hashValueNew = hashCodeNew % tableTemp.length;
				tableTemp[hashValueNew] = new LinkedList();
				tableTemp[hashValueNew].add(entry);
			}
		}
		table = tableTemp;
