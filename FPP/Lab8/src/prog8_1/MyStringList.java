...
public class MyStringList {
	...
	public void minSort(){
		...
		for(int i=0;i<strArray.length;++i) {
				...
				int minStrPosition=minpos(i,strArray.length-1);
				swap(i, minStrPosition);
	...
	public int minpos(int bottom, int top){		
		//implement
		int position=bottom;
		String minStrVal=strArray[bottom];
		for(int i=bottom+1;i<=top;++i) {
			
			if(strArray[i]!=null && strArray[i].compareTo(minStrVal)<0) {
				minStrVal=strArray[i];
				position=i;
			}
		}
		return position;
