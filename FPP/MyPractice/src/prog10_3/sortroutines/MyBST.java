...
public class MyBST {
	/** The tree root. */
	private Node root;
	private List<Integer> allInTogather = new ArrayList<Integer>();
	...
	private void convertTreeToSortArr(Node t) {
		if (t != null) {
			printTree(t.left);
			allInTogather.add(t.element);
			printTree(t.right);
		}
	}

	public int[] readIntoArray() {
		convertTreeToSortArr(root);
		int[] arr = new int[allInTogather.size()];
		for (int i = 0; i < arr.length; i++) {
			arr[i] = allInTogather.get(i);
		}
		return arr;
	}
