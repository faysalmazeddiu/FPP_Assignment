...
public class Driver {
		...
		while(!inputString.equalsIgnoreCase("N")){
			...
			if(inputString.equalsIgnoreCase("A")){
				...
				outputString = "In Input Area, type in the book id number using this format:"+n+
				   ...
				   "where 'xxx' is the 3-digit code for the employee number.";
				System.out.println(outputString);
				checkIfBookIsInStock(sc.nextLine());
				...
				System.out.println(outputString);	
				checkWhetherFoodItemInMarket(sc.nextLine());
	...
	void displayNumberOfEmployees() {
		...
		int numberOfEmployee=directory.getNumberOfBookstoreEmployees();
		if(numberOfEmployee>-1) {
			System.out.println("Number of employees is: " +numberOfEmployee +" (Look for other console messages.)"+n+CONTINUE);
		}
	...
	void addEmployee(String employeeId){
		...
		if(directory.addNewEmployee(employeeId)) {
			System.out.println("Employee Successfully added" + n+CONTINUE);
		}else {
			System.out.println("Sorry !! Employee is not added to Store" + n+CONTINUE);
		}
...
//		System.out.println("Employee added unless a console message says otherwise " + n+CONTINUE);
	...
	void addBook(String bookId){
		...
		if(directory.addNewBook(bookId)) {
			System.out.println("New Book Successfully added"+ n+CONTINUE);
		}else {
			System.out.println("Sorry !! Book is not added to Store"+ n+CONTINUE);
		}
	...
	void addCD(String cdId){
		...
		if(directory.addNewCD(Integer.parseInt(cdId))) {
			System.out.println("CD added to the store successfully " + n+CONTINUE);
		}else {
			System.out.println("sorry !! CD is not added to the store " + n+CONTINUE);
		}
	...
	void displayNumberOfCDsInBookstore(){
		//implement-- redo the implementation
		int numberOfCDBook=directory.getNumCDsInBookstore();
		System.out.println("Number of CD's is " + numberOfCDBook+" (Look for console messages.)"+n+CONTINUE);
